...
OpenManipulatorController::OpenManipulatorController(std::string usb_port,
    ...
    : node_handle_(""), priv_node_handle_("~"), timer_thread_state_(false) {
  /************************************************************
  ...
  ************************************************************/
  open_manipulator_.initOpenManipulator(using_platform_, usb_port, baud_rate,
                                        control_period_);

  if (using_platform_ == true)
    log::info("Succeeded to init " + priv_node_handle_.getNamespace());
  else if (using_platform_ == false)
    log::info("Ready to simulate " + priv_node_handle_.getNamespace() +
              " on Gazebo");
...
OpenManipulatorController::~OpenManipulatorController() {
  ...
  pthread_join(
      timer_thread_,
      NULL); // Wait for the thread associated with thread_p to complete
...
void OpenManipulatorController::startTimerThread() {
  ...
  //  if (error != 0)   log::error("pthread_attr_setschedpolicy error = ",
  //  (double)error); error = pthread_attr_setinheritsched(&attr_,
  //  PTHREAD_EXPLICIT_SCHED); if (error != 0)
  //  log::error("pthread_attr_setinheritsched error = ", (double)error);

  //  memset(&param, 0, sizeof(param));
  //  param.sched_priority = 31;    // RT
  //  error = pthread_attr_setschedparam(&attr_, &param);
  //  if (error != 0)   log::error("pthread_attr_setschedparam error = ",
  //  (double)error);

  //  if ((error = pthread_create(&this->timer_thread_, &attr_,
  //  this->timerThread, this)) != 0)
  //  {
  ...
  int error;
  if ((error = pthread_create(&this->timer_thread_, NULL, this->timerThread,
                              this)) != 0) {
    log::error("Creating timer thread failed!!", (double)error);
    exit(-1);
  }
...
void *OpenManipulatorController::timerThread(void *param) {
  ...
  while (controller->timer_thread_state_) {
    next_time.tv_sec +=
        (next_time.tv_nsec +
         ((int)(controller->getControlPeriod() * 1000)) * 1000000) /
        1000000000;
    next_time.tv_nsec =
        (next_time.tv_nsec +
         ((int)(controller->getControlPeriod() * 1000)) * 1000000) %
    ...
    // log::info("control time : ", controller->getControlPeriod() -
    // delta_nsec);
...
void OpenManipulatorController::initPublisher() {
  ...
  auto om_tools_name =
      open_manipulator_.getManipulator()->getAllToolComponentName();

  for (auto const &name : om_tools_name) {
    ros::Publisher pb;
    pb = node_handle_.advertise<open_manipulator_msgs::KinematicsPose>(
        name + "/kinematics_pose", 10);
    open_manipulator_kinematics_pose_pub_.push_back(pb);
  }
  open_manipulator_states_pub_ =
      node_handle_.advertise<open_manipulator_msgs::OpenManipulatorState>(
          "states", 10);

  if (using_platform_ == true) {
    open_manipulator_joint_states_pub_ =
        node_handle_.advertise<sensor_msgs::JointState>("joint_states", 10);
  } else {
    auto gazebo_joints_name =
        open_manipulator_.getManipulator()->getAllActiveJointComponentName();
    gazebo_joints_name.reserve(gazebo_joints_name.size() +
                               om_tools_name.size());
    gazebo_joints_name.insert(gazebo_joints_name.end(), om_tools_name.begin(),
                              om_tools_name.end());

    for (auto const &name : gazebo_joints_name) {
      ros::Publisher pb;
      pb = node_handle_.advertise<std_msgs::Float64>(name + "_position/command",
                                                     10);
...
void OpenManipulatorController::initSubscriber() {
  ...
  open_manipulator_option_sub_ = node_handle_.subscribe(
      "option", 10, &OpenManipulatorController::openManipulatorOptionCallback,
      this);
}

void OpenManipulatorController::initServer() {
  goal_joint_space_path_server_ = node_handle_.advertiseService(
      "goal_joint_space_path",
      &OpenManipulatorController::goalJointSpacePathCallback, this);
  ...
  goal_task_space_path_server_ = node_handle_.advertiseService(
      "goal_task_space_path",
      &OpenManipulatorController::goalTaskSpacePathCallback, this);
  goal_task_space_path_position_only_server_ = node_handle_.advertiseService(
      "goal_task_space_path_position_only",
      &OpenManipulatorController::goalTaskSpacePathPositionOnlyCallback, this);
  goal_task_space_path_orientation_only_server_ = node_handle_.advertiseService(
      "goal_task_space_path_orientation_only",
      &OpenManipulatorController::goalTaskSpacePathOrientationOnlyCallback,
      this);

  goal_joint_space_path_from_present_server_ = node_handle_.advertiseService(
      "goal_joint_space_path_from_present",
      &OpenManipulatorController::goalJointSpacePathFromPresentCallback, this);

  goal_task_space_path_from_present_server_ = node_handle_.advertiseService(
      "goal_task_space_path_from_present",
      &OpenManipulatorController::goalTaskSpacePathFromPresentCallback, this);
  ...
  goal_tool_control_server_ = node_handle_.advertiseService(
      "goal_tool_control", &OpenManipulatorController::goalToolControlCallback,
      this);
  set_actuator_state_server_ = node_handle_.advertiseService(
      "set_actuator_state",
      &OpenManipulatorController::setActuatorStateCallback, this);
  goal_drawing_trajectory_server_ = node_handle_.advertiseService(
      "goal_drawing_trajectory",
      &OpenManipulatorController::goalDrawingTrajectoryCallback, this);
}
...
bool OpenManipulatorController::goalJointSpacePathToKinematicsPoseCallback(
  ...
  if (!open_manipulator_.makeJointTrajectory(req.end_effector_name, target_pose,
                                             req.path_time))
    res.is_planned = false;
  else
    res.is_planned = true;
...
bool OpenManipulatorController::goalJointSpacePathToKinematicsPositionCallback(
  ...
  if (!open_manipulator_.makeJointTrajectory(
          req.end_effector_name, target_pose.position, req.path_time))
    res.is_planned = false;
  else
    res.is_planned = true;
...
bool OpenManipulatorController::
    goalJointSpacePathToKinematicsOrientationCallback(
        open_manipulator_msgs::SetKinematicsPose::Request &req,
        open_manipulator_msgs::SetKinematicsPose::Response &res) {
  KinematicPose target_pose;
  ...
  if (!open_manipulator_.makeJointTrajectory(
          req.end_effector_name, target_pose.orientation, req.path_time))
    res.is_planned = false;
  else
    res.is_planned = true;
  return true;
...
bool OpenManipulatorController::goalTaskSpacePathCallback(
  ...
  target_pose.orientation = math::convertQuaternionToRotationMatrix(q);
  if (!open_manipulator_.makeTaskTrajectory(req.end_effector_name, target_pose,
                                            req.path_time))
    res.is_planned = false;
  else
    res.is_planned = true;
...
bool OpenManipulatorController::goalTaskSpacePathPositionOnlyCallback(
  ...
  if (!open_manipulator_.makeTaskTrajectory(req.end_effector_name, position,
                                            req.path_time))
    res.is_planned = false;
  else
    res.is_planned = true;
...
bool OpenManipulatorController::goalTaskSpacePathOrientationOnlyCallback(
  ...
  if (!open_manipulator_.makeTaskTrajectory(req.end_effector_name, orientation,
                                            req.path_time))
    res.is_planned = false;
  else
    res.is_planned = true;
...
bool OpenManipulatorController::goalJointSpacePathFromPresentCallback(
  ...
  if (!open_manipulator_.makeJointTrajectoryFromPresentPosition(target_angle,
                                                                req.path_time))
    res.is_planned = false;
  else
    res.is_planned = true;
...
bool OpenManipulatorController::goalTaskSpacePathFromPresentCallback(
  ...
  if (!open_manipulator_.makeTaskTrajectoryFromPresentPose(
...
bool OpenManipulatorController::
  ...
  if (!open_manipulator_.makeTaskTrajectoryFromPresentPose(
          req.planning_group, position, req.path_time))
    res.is_planned = false;
  else
    res.is_planned = true;
...
bool OpenManipulatorController::goalToolControlCallback(
    ...
    open_manipulator_msgs::SetJointPosition::Response &res) {
  for (int i = 0; i < req.joint_position.joint_name.size(); i++) {
    if (!open_manipulator_.makeToolTrajectory(
            req.joint_position.joint_name.at(i),
            req.joint_position.position.at(i)))
      res.is_planned = false;
    else
      res.is_planned = true;
  }
...
bool OpenManipulatorController::setActuatorStateCallback(
    open_manipulator_msgs::SetActuatorState::Request &req,
...
bool OpenManipulatorController::goalDrawingTrajectoryCallback(
    open_manipulator_msgs::SetDrawingTrajectory::Request &req,
  ...
  try {
    if (req.drawing_trajectory_name == "circle") {
      ...
      if (!open_manipulator_.makeCustomTrajectory(
              CUSTOM_TRAJECTORY_CIRCLE, req.end_effector_name,
              p_draw_circle_arg, req.path_time))
        res.is_planned = false;
      else
        res.is_planned = true;
      ...
      if (!open_manipulator_.makeCustomTrajectory(
              CUSTOM_TRAJECTORY_LINE, req.end_effector_name, p_draw_line_arg,
              req.path_time))
        res.is_planned = false;
      else
        res.is_planned = true;
      ...
      if (!open_manipulator_.makeCustomTrajectory(
              CUSTOM_TRAJECTORY_RHOMBUS, req.end_effector_name,
              p_draw_rhombus_arg, req.path_time))
        res.is_planned = false;
      else
        res.is_planned = true;
      ...
      if (!open_manipulator_.makeCustomTrajectory(
              CUSTOM_TRAJECTORY_HEART, req.end_effector_name, p_draw_heart_arg,
              req.path_time))
        res.is_planned = false;
      else
        res.is_planned = true;
...
void OpenManipulatorController::publishKinematicsPose() {
  ...
  for (auto const &tools : om_tools_name) {
    ...
    Eigen::Quaterniond orientation =
        math::convertRotationMatrixToQuaternion(pose.orientation);
...
int main(int argc, char **argv) {
  ...
  if (argc = 3) {
    ...
    log::error("Please set '-port_name' and  '-baud_rate' arguments for "
  ...
  ros::Timer publish_timer = node_handle.createTimer(
      ros::Duration(om_controller.getControlPeriod()),
      &OpenManipulatorController::publishCallback, &om_controller);
